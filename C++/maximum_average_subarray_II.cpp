...
/*
 ...
 Now, we need to find if a subarray with length greater than or equal to k is possible with an average sum greater than this midmid value.

 To determine if this is possible in a single scan, let's look at an observation. Suppose, there exist j elements, a_1, a_2, a_3, ... a_j 
 ...
 Thus, we can see that if after subtracting the midmid number from the elements of a subarray with more than k-1 elements, within nums,
...
Thus, in this case, we need to set the mid as the new minimum element and continue the process.
 ...
 If on traversing the first k elements, the sum becomes greater than or equal to 0, we can directly determine that we can increase the average beyond mid. 
 ...
 Further, for sum_i as the cumulative sum up to the current(ith) index, all we need is sum_j - sum_i ≥ 0 such that j - i ≥ k.

 To achive this, instead of checking with all possible values of sum_i, 
 we can just consider the minimum cumulative sum upto the index j - k. This is because if the required condition can't be sastisfied with the minimum sum_i, 
 ...
 In order to keep our precision in control, we limit this process to 10^-510
 −
