        ...
        for (auto& it : move) {
            ...
            while (curX + xDiff < M and curX + xDiff >= 0
                ...
                if (curX == hole[0] and curY == hole[1]) {
                    ...
                    else if (distance[hole[0]][hole[1]] == distance[ball[0]][ball[1]] + length) {
                        directions = min(directions, path);
                    }
                    drop = true;
                    break;
                }
...
public:
    string findShortestWay(vector<vector<int>>& maze, vector<int>& ball, vector<int>& hole) {
        ...
        string directions = "";
        string path = "";
        dfs(maze, {ball[0], ball[1]}, hole, path, directions, distance, move);
        return directions == "" ? "impossible" : directions;
    }
};
