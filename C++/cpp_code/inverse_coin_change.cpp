...
You are given a 1-indexed integer array numWays, where numWays[i] represents the number of ways to select a total amount
i using an infinite supply of some fixed coin denominations. Each denomination is a positive integer with value at most
numWays.length.

However, the exact coin denominations have been lost. Your task is to recover the set of denominations that could have
resulted in the given numWays array.
...
public:
    vector<int> findCoins(vector<int>& numWays) {
        int n = numWays.size();
        vector<long long> my_ways(n + 1, 0);
        my_ways[0] = 1;
        vector<int> rst;
        for (int i = 1; i <= n; i++) {
            if (my_ways[i] == numWays[i - 1]) {
                continue;
            }

            if (numWays[i - 1] == 1 + my_ways[i]) {
                rst.push_back(i);
                for (int j = i; j <=n; j++) {
