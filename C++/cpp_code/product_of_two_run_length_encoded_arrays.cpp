...
way to read this is "three 1's followed by five 2's". The product of two run-length encoded arrays encoded1 and encoded2
...
public:
    vector<vector<int>> findRLEArray(vector<vector<int>>& encoded1, vector<vector<int>>& encoded2) {
        ...
        while (idx1 < n1 && idx2 < n2) {
            int product = encoded1[idx1][0] * encoded2[idx2][0];
            int min_freq = min(encoded1[idx1][1], encoded2[idx2][1]);
            if (!rst.empty() && rst.back()[0] == product) {
                rst.back()[1] += min_freq;
            } else {
                rst.push_back({product, min_freq});
            }
