...
public:
    long long taskSchedulerII(vector<int>& tasks, int space) {
        int n = tasks.size();
        long long cur_date = 0;
        unordered_map<int, long long> last_date;
        for (int i = 0; i < n; i++) {
            int task_type = tasks[i];
            if (last_date.find(task_type) == last_date.end() || cur_date - last_date[task_type] > space) {
                cur_date += 1;
            } else {
                cur_date = last_date[task_type] + space + 1;
            }
            last_date[task_type] = cur_date;
        }
        return cur_date;
    }
};
