/*
给你一个二维数组（2D array），实现一个矩形区域拷贝操作——把数组中某个矩形区域的内容复制到另一个位置，要求额外内存只能用 O(1)，也就是不能开一个临时矩阵来暂存。
比如在一个 8×8 的网格上，把左上角 3×3 的区域复制到右下角某个位置：
源区域 (r1=0, c1=0, 3x3)     目标位置 (r2=5, c2=5)
