    ...
    int shortestPathAllKeys(vector<string> &grid) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                ...
                } else if (grid[i][j] >= 'A' and grid[i][j]
                <=
                'F'
                )
                {
        ...
        while (!bfs.empty()) {
            ...
            for (int i = 0; i < curSize; i++) {
                ...
                for (int d = 0; d < 4; d++) {
                    ...
                    if (nr < 0 or nr
                    >=
                    m or nc<0 or nc >= n or grid[nr][nc] == '#'
                    )
                    {
                        continue;
                    }

                    if (grid[nr][nc] >= 'A' and grid[nr][nc]
                    <=
                    'F'
                    )
                    {
                    ...
                    int pos = nr * n + nc;
                    if (visited.find(pos) == visited.end() or visited[pos]
                    .
                    find(have) == visited[pos].end()
                    )
                    {
