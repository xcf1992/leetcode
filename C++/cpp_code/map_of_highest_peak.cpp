...
public:
    vector<vector<int>> highestPeak(vector<vector<int>>& isWater) {
        ...
        while (!bfs.empty()) {
            int curI = bfs.front()[0];
            int curJ = bfs.front()[1];
            int curH = bfs.front()[2];
            bfs.pop();

            for (int i = 0; i < 4; ++i) {
                int nexI = curI + diff[i];
                int nexJ = curJ + diff[i + 1];

                if (nexI < 0 || nexJ < 0 || nexI >= m || nexJ >= n || result[nexI][nexJ] != -1) {
                    continue;
                }

                result[nexI][nexJ] = curH + 1;
                bfs.push({nexI, nexJ, result[nexI][nexJ]});
            }
        }
        return result;
    }
