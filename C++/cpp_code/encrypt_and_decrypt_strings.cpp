...
Note that in case a character of the string is not present in keys, the encryption process cannot be carried out, and an
empty string "" is returned.

A string is decrypted with the following process:

For each substring s of length 2 occurring at an even index in the string, we find an i such that values[i] == s. If
there are multiple valid i, we choose any one of them. This means a string could have multiple possible strings it can
decrypt to. Replace s with keys[i] in the string. Implement the Encrypter class:
...
["abcd"], ["eizfeiam"]] Output [null, "eizfeiam", 2]

Explanation
Encrypter encrypter = new Encrypter([['a', 'b', 'c', 'd'], ["ei", "zf", "ei", "am"], ["abcd", "acbd", "adbc", "badc",
"dacb", "cadb", "cbda", "abad"]); encrypter.encrypt("abcd"); // return "eizfeiam".
...
public:
    Encrypter(vector<char>& keys, vector<string>& values, vector<string>& dictionary) {
        for (int i = 0; i < keys.size(); i++) {
            key_to_val_[keys[i]] = values[i];
        }

        for (string& word : dictionary) {
            encrypt_cnt[encrypt(word)] += 1;
        }
    }
    ...
    string encrypt(string word1) {
        string rst = "";
        for (char c : word1) {
            if (key_to_val_.find(c) == key_to_val_.end()) {
                return "";
            }
            rst += key_to_val_[c];
        }
    ...
    int decrypt(string word2) {
        if (encrypt_cnt.find(word2) == encrypt_cnt.end()) {
            return 0;
        }
        return encrypt_cnt[word2];
    }
};
