...
Now as I have to eat all the candies of type < i before eating candy of type i,
so we can have a prefix sum array for candies count
...
public:
    vector<bool> canEat(vector<int>& candies, vector<vector<int>>& queries) {
        ...
        for (auto& query : queries) {
            ...
            long long max_day = prefix[type + 1] - 1;
            long long minDay = prefix[type] / cap;

            if (day <= max_day && day >= minDay) {
