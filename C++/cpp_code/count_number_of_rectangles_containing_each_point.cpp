...
public:
    vector<int> countRectangles(vector<vector<int>>& rectangles, vector<vector<int>>& points) {
        sort(rectangles.begin(), rectangles.end(), [](vector<int> a, vector<int> b) { return a[1] < b[1]; });
...
public:
    ...
    int binsear(vector<int>& arr, int x) {
        ...
        // if we find no m such that arr[m]>=x, means that our x is greater than all values of length, in such case we
        // return idx n (i.e. size of arr) reason being, we substract our idx by size of arr to get number of rectangles
        // greater than that in main fxn, so return arr.size would give us 0 (which we want)
