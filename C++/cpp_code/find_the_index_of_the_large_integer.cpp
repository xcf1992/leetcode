...
rest of the integers. You will not be given direct access to the array, instead, you will have an API ArrayReader which
...
arr[l]+arr[l+1]+...+arr[r] > arr[x]+arr[x+1]+...+arr[y]. 0 if arr[l]+arr[l+1]+...+arr[r] == arr[x]+arr[x+1]+...+arr[y].
...
(i.e. compares arr[0] with arr[1]). Thus we know that arr[0] and arr[1] doesn't contain the largest element.
...
public:
    int getIndex(ArrayReader& reader) {
        ...
        while (left < right) {
            ...
            if (reader.compareSub(left, left + mid - 1, left + mid, left + 2 * mid - 1) != 1) {
                left = left + mid;
            } else {
                right = left + mid - 1;
            }
