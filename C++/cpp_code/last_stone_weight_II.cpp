...
If y_1 > z_1, consider ((y_1-z_1)+(y_2+...+y_m)) - (z_2+...+z_n); we know that it is >= 0, so it is >= the minimum
a(y_1-z_1, y_2,...,y_m, z_2,...,z_n) (Ineq 1). By induction hypothesis (we've reduced the number of stones by one),
a(y_1-z_1, y_2,...,y_m, z_2,...,z_n) = b(y_1-z_1, y_2,...,y_m, z_2,...,z_n) (Eq 2). Note that b(x_1, ..., x_k) is the
minimum among all first-step choices of two stones to cancel. Since {y_1,z_1} is one possible choice, we have b(y_1-z_1,
y_2,...,y_m, z_2,...,z_n) >= b(x_1, ..., x_k) (Ineq 2). Combining (Eq 1), (Ineq 1), (Eq 2) and (Ineq 2), we get
a(x_1,...,x_k) >= b(x_1,...,x_k), hence a(x_1,...,x_k) = b(x_1,...,x_k) by (Ineq 0). If y_1 < z_1, consider
(y_2+...+y_m) - ((z_1-y_1)+z_2+...+z_n) and the same reasoning applies. If y_1 = z_1, consider (y_2+...+y_m) -
(z_2+...+z_n) and the same reasoning applies. In the last case we reduce the number of stones by two. We've shown that
a=b for any k stones given that a=b for any (k-1) stones and any (k-2) stones. By induction we know a=b for any number
of stones.

=*=*=*=*=*=*==*=*=*==*=*=*==*=*=*==*=*=*==*=*=*==*=*=*==*=*=*==*=*=*==*=*=*==*=*=*==*=*=*==*=*=*==*=*=*==*=*=*=*=*=*==*=*=*==*=*=*==*=*=*==*=*=*==*=*=*==*=*=*==*=*=*==*=*=*==*=*=*==*=*=*==*=*=*==*=*=*==*=*=*=
Proof that the max final weight is (the max stone) - (the min final stone from the remaining stones):
Let J - K be the last cancellation to get the max final stone, where J and K each is an original stone or obtained from
some cancellations. I claim that J must be an original stone: otherwise J = L - H and hence J - K = L - (H+K). But then
L - |H-K| is larger and can also be obtained, contradicting our assumption. It remains to show that J - (min final stone
from the remaining) achieves maximum when L is the max stone M. If J is not the max, the max stone M appears in K when K
is written as (sum of some stones) - (sum of remaining stones). So K = M + K1 - K2 or K = K1 - K2 - M.

If K = K1 - K2 - M, we have J - K = J - (K1 - K2 - M) = M - (K1 - K2 - J). Since K1 - K2 - M = K >= 0 and M > J, we have
K1 - K2 - J > 0, so the minimum final stone obtained from the stones in K1, K2 and J is at most K1 - K2 - J, so M - (min
final from J,K1,K2) >= M - (K1 - K2 - J) = J - K. If K = M + K1 - K2, we have J - K = J - (M + K1 - K2). Now consider M
- (J + K1 - K2). We know M + K1 - K2 = K >= 0, but J + K1 - K2 may be < 0. (In fact it must be if J-K is the max,
otherwise M-(J+K1-K2) >= J-(M+K1-K2) contradicting maximality.) However, we still know that the minimum final stone from
stones in K1, K2 and J is at most |J+K1-K2|, and it is always true that |J+K1-K2| <= (M+K1-K2) + (M-J) (a special case
of the triangle inequality |a+b| <= |a| + |b|). So M - (min final from J,K1,K2) >= M - |J+K1-K2| >= M - (M-J) -
(M+K1-K2) = J - K.
*/
class Solution {
public:
...
/*
 1. Think of the final answer as a sum of weights with + or - sign symbols infront of each weight. Actually, all sums
 with 1 of each sign symbol are possible.
 2. Use dynamic programming: for every possible sum with N stones, those sums +x or -x is possible with N+1 stones,
 where x is the value of the newest stone. (This overcounts sums that are all positive or all negative, but those don't
 matter.)
