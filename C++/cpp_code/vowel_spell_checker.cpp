...
public:
    vector<string> spellchecker(vector<string>& wordlist, vector<string>& queries) {
        ...
        for (int i = 0; i < wordlist.size(); i++) {
            string lowerWord = toLowerCase(wordlist[i]);
            if (capMatch.find(lowerWord) == capMatch.end()) {
                capMatch[lowerWord] = wordlist[i];
            }

            string deWord = devowel(lowerWord);
            if (vowelMatch.find(deWord) == vowelMatch.end()) {
                vowelMatch[deWord] = wordlist[i];
            }
        }

        for (int i = 0; i < queries.size(); i++) {
            string word = queries[i];
            if (exactlyMatch.find(word) != exactlyMatch.end()) {
                continue;
            }

            string loWord = toLowerCase(word);
            if (capMatch.find(loWord) != capMatch.end()) {
                queries[i] = capMatch[loWord];
                continue;
            }

            string deWord = devowel(loWord);
            if (vowelMatch.find(deWord) != vowelMatch.end()) {
                queries[i] = vowelMatch[deWord];
                continue;
            }
            queries[i] = "";
        }
