...
may be incorrect. Another record with the same timestamp may appear later in the stream correcting the price of the
...
timestamp. Finds the latest price of the stock based on the current records. The latest price is the price at the latest
timestamp recorded. Finds the maximum price the stock has been based on the current records. Finds the minimum price the
stock has been based on the current records. Implement the StockPrice class:
...
public:
    ...
    void update(int timestamp, int price) {
        time_to_price_[timestamp] = price;
        max_price_.push(make_pair(price, timestamp));
        min_price_.push(make_pair(price, timestamp));
    }

    int current() {
        return time_to_price_.rbegin()->second;
    }
