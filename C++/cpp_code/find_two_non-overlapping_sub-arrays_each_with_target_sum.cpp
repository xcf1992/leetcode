    ...
    int minSumOfLengths(vector<int>& arr, int target) {
        ...
        for (int i = 0; i < arr.size(); i++) {
            ...
            if (hmap.find(sum - target) != hmap.end()) {
                ...
                // stores minimum length of sub-array starting with index<= i with sum target. This ensures non-
            ...
            if (hmap.find(sum + target) != hmap.end() and lsize < INT_MAX) {
                result =
                        min(result, hmap[sum + target] - i +
                                            lsize);  // updates the result only if both left and right sub-array exists.
            }
        }
