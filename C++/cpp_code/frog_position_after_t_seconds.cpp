...
vertex 2 after second 1 and then jumping with 1/2 probability to vertex 4 after second 2. Thus the probability for the
frog is on the vertex 4 after 2 seconds is 1/3 * 1/2 = 1/6 = 0.16666666666666666.

Example 2:
Input: n = 7, edges = [[1,2],[1,3],[1,7],[2,4],[2,6],[3,5]], t = 1, target = 7
Output: 0.3333333333333333
Explanation: The figure above shows the given graph. The frog starts at vertex 1, jumping with 1/3 = 0.3333333333333333
...
We can use a simple DFS to reach the target node while storing the probablity of reaching that node at t by multiplying
by the number of children that node has. Once we reach that target and depth required in the problem we can return that
probablity. Edge cases:

When when either the node is somewhere in the middle and we reach that node but not in target time, we simply return 0.
When it's a leaf node but the time is less than the required one, it'll simply jump until the goal time, thus we can
return the same probablity.
*/
class Solution {
private:
    vector<vector<int>> G;
