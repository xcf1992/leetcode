...
that will expire timeToLive seconds after the currentTime.
If the token is renewed, the expiry time will be extended to expire timeToLive seconds
...
If there are no unexpired tokens with the given tokenId,
...
and another action happens on time t (renew or countUnexpiredTokens),
