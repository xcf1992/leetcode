...
lexicographically by their identifiers. The digit-logs maintain their relative ordering. Return the final order of the
logs.
...
public:
    vector<string> reorderLogFiles(vector<string>& logs) {
        ...
        sort(letter_logs.begin(), letter_logs.end(), [](string& a, string& b) {
            string content_a = a.substr(a.find(" ") + 1);
            string content_b = b.substr(b.find(" ") + 1);
            return content_a < content_b || (content_a == content_b && a < b);
        });

        for (string log : digit_logs) {
            letter_logs.push_back(log);
        }
        return letter_logs;
    }
