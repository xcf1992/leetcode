...
class Solution {
private:
    int get_complete_time(vector<vector<int>>& prev_courses, vector<int>& complete_time, vector<int>& time,
                          int course) {
        if (complete_time[course] != -1) {
            return complete_time[course];
        }

        if (prev_courses[course].size() == 0) {
            complete_time[course] = time[course];
            return complete_time[course];
        }

        for (int prev : prev_courses[course]) {
            complete_time[course] = max(complete_time[course],
                                        time[course] + get_complete_time(prev_courses, complete_time, time, prev));
        }
        return complete_time[course];
    }
...
public:
    int minimumTime(int n, vector<vector<int>>& relations, vector<int>& time) {
        vector<vector<int>> prev_courses(n, vector<int>());
        for (vector<int>& relation : relations) {
            int prev = relation[0] - 1;
            int next = relation[1] - 1;
            prev_courses[next].push_back(prev);
        }
