...
using namespace std;
/*
I am not able to pass this question one time but struggle a lot in the basic test cases like "a", "aa", "aaa", "aba",
"aabb". Those test cases help my early rough idea to be flawless. The basic idea of DP is easy to understand, I maintain
...
private:
    ...
    void update(int i, int j, vector<vector<int>>& dp, string& S) {
        ...
        if (left > right) {
             ...
             * the reason why dp[i + 1][j  - 1] * 2 counted is that we count dp[i + 1][j - 1] one time as {"a",  "c",
             * "aa", "aca"}, and additional time as {"aaa",  "aca", "aaaa", "aacaa"}. Now there is duplicate :  {"aca"},
