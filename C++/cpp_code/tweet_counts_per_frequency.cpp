...
(depending on freq) starting from the startTime (in seconds)
and ending at the endTime (in seconds).
freq is always minute, hour or day,
representing the time interval to get the total number of occurrences for the given tweetName.
...
public:
    ...
    vector<int> getTweetCountsPerFrequency(string freq, string tweetName, int startTime, int endTime) {
        ...
        vector<int> result((endTime - startTime) / gap + 1, 0);
        const auto target = tweets.find(tweetName);
        if (target == tweets.end()) {
            return result;
        }
