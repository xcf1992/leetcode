...
public:
    int minimumScore(string s, string t) {
        ...
        int t_len = t.size();
        int k = t_len - 1;
        // dp[i] store the s_idx, where t[i ... t_len - 1] is subsequence of s[dp[i]...s_len - 1]
        vector<int> dp(t_len, -1);
        for (int i = s_len - 1; i >= 0 && k >= 0; --i) {
            if (s[i] == t[k]) {
                dp[k] = i;
                k -= 1;
            }
        ...
        for (int s_idx = 0, t_idx = 0; s_idx < s_len && t_idx < t_len && rst > 0; ++s_idx) {
            if (s[s_idx] == t[t_idx]) {
                while (k < t.size() && dp[k] <= s_idx) {
                    k += 1;
                }
                t_idx += 1;
                rst = min(rst, (k - 1) - t_idx + 1);
            }
        }
        return rst;
    }
