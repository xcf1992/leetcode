...
class Solution {
private:
    bool can_complete(vector<int>& time, long long total_time, int total_trips) {
        long long trip_cnt = 0;
        for (const int& time_cost : time) {
            trip_cnt += total_time / time_cost;
            if (trip_cnt >= total_trips) {
                return true;
            }
        }
        return false;
