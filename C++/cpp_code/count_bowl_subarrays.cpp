...
The minimum of its two ends is strictly greater than the maximum of all elements in between. That is, min(nums[l],
nums[r]) > max(nums[l + 1], ..., nums[r - 1]). Return the number of bowl subarrays in nums.
    ...
    long long bowlSubarrays(vector<int>& nums) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            while (!min_stk.empty() && nums[i] > nums[min_stk.top()]) {
                if (i - min_stk.top() + 1 >= 3) {
                    rst += 1;
                }
                min_stk.pop();
            }

            if (!min_stk.empty() && i - min_stk.top() + 1 >= 3) {
                rst += 1;
            }
            min_stk.push(i);
        }
