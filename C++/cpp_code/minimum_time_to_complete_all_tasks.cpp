/*
https://leetcode.com/problems/minimum-time-to-complete-all-tasks/description/?envType=company&envId=snapchat&favoriteSlug=snapchat-all
...
where tasks[i] = [starti, endi, durationi] indicates that the ith task should run for a total of durationi seconds (not
    ...
    int findMinimumTime(vector<vector<int>>& tasks) {
        ...
        for (auto& task : tasks) {
            ...
            for (int t = task[0]; t <= task[1]; t++) {
                if (used.count(t)) {
                    used_time += 1;  // have used this time slice
                }
            }

            for (int t = task[1]; used_time < task[2]; t--) {
