...
then it gets out at time t2 with t2 > t1.
...
"Paradise" (at time 8) to "Cambridge" (at time 22) undergroundSystem.getAverageTime("Leyton", "Waterloo");          //
return 11.0. There were two travels from "Leyton" to "Waterloo", a customer with id=45 from time=3 to time=15 and a
customer with id=27 from time=10 to time=20. So the average time is ( (15-3) + (20-10) ) / 2 = 11.0
...
public:
    ...
    void checkOut(int id, string stationName, int t) {
        double duration = t - checkInTime[id];
        string key = checkInStation[id] + "_" + stationName;
        avgTime[key] += duration;
        count[key] += 1;
    }
