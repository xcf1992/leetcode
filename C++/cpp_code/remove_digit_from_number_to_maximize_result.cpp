...
/*
Instead, we can just remove the leftmost digit if it's followed by a larger number. If we cannot find such case, we
remove the last occurence of digit.
 */
class Solution {
public:
    string removeDigit(string num, char digit) {
        ...
        int last_d = num.rfind(digit);
        return num.substr(0, last_d) + num.substr(last_d + 1);
    }
};