...
Explanation: There are a total of 9 spaces and 4 words. We can evenly divide the 9 spaces between the words: 9 / (4-1) =
3 spaces.
...
public:
    string reorderSpaces(string text) {
        ...
        string spaces = string(count / (words.size() - 1), ' ');
        for (int i = 1; i < words.size(); ++i) {
            result += spaces;
            result += words[i];
        }
        return result + string(count % (words.size() - 1), ' ');
    }
};