...
You want to find the minimum distance to the target cell. However, you do not know the grid's dimensions, the starting
cell, nor the target cell. You are only allowed to ask queries to the GridMaster object.

Thr GridMaster class has the following functions:

boolean canMove(char direction) Returns true if the robot can move in that direction. Otherwise, it returns false.
void move(char direction) Moves the robot in that direction. If this move would move the robot to a blocked cell or off
the grid, the move will be ignored, and the robot will remain in the same position. boolean isTarget() Returns true if
the robot is currently on the target cell. Otherwise, it returns false. Note that direction in the above functions
should be a character from {'U','D','L','R'}, representing the directions up, down, left, and right, respectively.
...
private:
    ...
    void dfs(int row, int col, vector<vector<int>>& grid, GridMaster& master) {
        if (master.isTarget()) {
            dest_row_ = row;
            dest_col_ = col;
            grid[row][col] = 2;
...
public:
    int findShortestPath(GridMaster& master) {
        ...
        dfs(src_row, src_col, grid, master);
        if (dest_row_ == -1) {
            return -1;
        }

        queue<pair<int, int>> bfs;
        bfs.push({src_row, src_col});
        unordered_set<int> visited;
        visited.insert(src_row * max_size + src_col);
        ...
        while (!bfs.empty()) {
            ...
            for (size_t i = 0; i < cur_size; i++) {
                ...
                for (int j = 0; j < 4; j++) {
                    int next_row = row + diff_[j];
                    int next_col = col + diff_[j + 1];
                    if (grid[next_row][next_col] != 0 &&
                        visited.find(next_row * max_size + next_col) == visited.end()) {
                        visited.insert(next_row * max_size + next_col);
