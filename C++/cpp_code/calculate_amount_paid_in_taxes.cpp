...
And so on.
You are given an integer income representing the amount of money you earned. Return the amount of money that you have to
...
3rd tax bracket. The tax rate for the three tax brackets is 50%, 10%, and 25%, respectively. In total, you pay $3 * 50%
+ $4 * 10% + $3 * 25% = $2.65 in taxes. Example 2:
...
public:
    double calculateTax(vector<vector<int>>& brackets, int income) {
        ...
        for (vector<int>& bracket : brackets) {
            ...
            double rate = (double)bracket[1] / 100;
            if ((double)income >= cap) {
                rst += (cap - paid) * rate;
                paid += cap - paid;
            } else {
                rst += ((double)income - paid) * rate;
                break;
            }
        }
        return rst;
