...
For example, if the ith server is busy,
...
/*
Intuition
We need to avoid scanning through k servers for every request. So, instead looping through requests, we will loop
through servers and take all requests that are available (not picked up by previous servers) and can be handled.

Solution
For each server, we track when it becomes available (avail), and also the count cnt of executed requests. We will use
that count in the end to determine the busiest server.

For a current server i % k, we add request i to the list of available requests. If we can process that request, we
remove it from the queue. If not - that request will become available for the next server to pick up. We will pick an
earlier request first - to ensure that a request is processed by the first available server.

Here, we are using ordered map to 'queue' available requests, so it's efficient to find requests that the current server
can process. We can further optimize by removing 'dropped' requests.
*/
class Solution {
public:
