...
We are going to construct a binary search tree (BST) by inserting the elements of nums in order into an initially empty
BST. Find the number of different ways to reorder nums so that the constructed BST is identical to that formed from the
original array nums.
...
We can find the left subtree elements which are all the elements that is smaller than root value, and right subtree
elements which are greater than root value.

And in order to make it identical with original BST, we should keep the relative order in left subtree elements and in
right subtree elements.

Assume the lenght of left subtree elements is left_len and right is right_len, they can change their absolute position
but need to keep their relative position in either left subtree or right right subtree.
...
// in code, we use Pascal triangle to keep a table of permutations, so we can look up the table and get permutation
result in O(1)
*/
class Solution {
private:
