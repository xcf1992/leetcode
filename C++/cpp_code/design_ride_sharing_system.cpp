...
int[] matchDriverWithRider() Matches the earliest available driver with the earliest waiting rider and removes both of
them from the system. Returns an integer array of size 2 where result = [driverId, riderId] if a match is made. If no
match is available, returns [-1, -1]. void cancelRider(int riderId) Cancels the ride request of the rider with the given
...
At most 1000 calls will be made in total to addRider​​​​​​​, addDriver, matchDriverWithRider, and
cancelRider.
*/
#include <iostream>
#include <string>
...
private:
    queue<int> riders_;
    queue<int> drivers_;
    unordered_set<int> cancelled_riders_;
...
public:
    ...
    void addRider(int riderId) {
        cancelled_riders_.erase(riderId);
    ...
    vector<int> matchDriverWithRider() {
        while (!riders_.empty() && cancelled_riders_.find(riders_.front()) != cancelled_riders_.end()) {
            cancelled_riders_.erase(riders_.front());
            riders_.pop();
        }

        if (!riders_.empty() && !drivers_.empty()) {
            int rider = riders_.front();
            riders_.pop();
    ...
    void cancelRider(int riderId) {
        cancelled_riders_.insert(riderId);
    }
};
