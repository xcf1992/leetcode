...
You have a video sharing platform where users can upload and delete videos. Each video is a string of digits, where the
ith digit of the string represents the content of the video at minute i. For example, the first digit represents the
content at minute 0 in the video, the second digit represents the content at minute 1 in the video, and so on. Viewers
of videos can also like and dislike videos. Internally, the platform keeps track of the number of views, likes, and
dislikes on each video.

When a video is uploaded, it is associated with the smallest available integer videoId starting from 0. Once a video is
deleted, the videoId associated with that video can be reused for another video.
...
String watch(int videoId, int startMinute, int endMinute) If there is a video associated with videoId, increase the
number of views on the video by 1 and return the substring of the video string starting at startMinute and ending at
min(endMinute, video.length - 1) (inclusive). Otherwise, return "-1". void like(int videoId) Increases the number of
likes on the video associated with videoId by 1 if there is a video associated with videoId. void dislike(int videoId)
Increases the number of dislikes on the video associated with videoId by 1 if there is a video associated with videoId.
int[] getLikesAndDislikes(int videoId) Return a 0-indexed integer array values of length 2 where values[0] is the number
of likes and values[1] is the number of dislikes on the video associated with videoId. If there is no video associated
with videoId, return [-1]. int getViews(int videoId) Return the number of views on the video associated with videoId, if
...
videoSharingPlatform.dislike(1);             // Increase the number of dislikes on the video associated with videoId 1.
videoSharingPlatform.getLikesAndDislikes(1); // There is 1 like and 2 dislikes on the video associated with videoId 1,
so return [1, 2]. videoSharingPlatform.getViews(1);            // The video associated with videoId 1 has 2 views, so
return 2. Example 2:
...
private:
    int max_video_id_;
    set<int> available_video_ids_;
    unordered_map<int, string> video_data_;
...
public:
    ...
    int upload(string video) {
        int video_id = max_video_id_;
        if (available_video_ids_.empty()) {
            max_video_id_ += 1;
        } else {
            video_id = *available_video_ids_.begin();
            available_video_ids_.erase(video_id);
        }

        video_data_[video_id] = video;
        view_cnt_[video_id] = 0;
        like_cnt_[video_id] = 0;
        dislike_cnt_[video_id] = 0;
        return video_id;
    }

    void remove(int videoId) {
        if (video_data_.find(videoId) == video_data_.end()) {
            return;
        }

        video_data_.erase(videoId);
        view_cnt_.erase(videoId);
        like_cnt_.erase(videoId);
        dislike_cnt_.erase(videoId);
        available_video_ids_.insert(videoId);
    }
    ...
    string watch(int videoId, int startMinute, int endMinute) {
        ...
        view_cnt_[videoId] += 1;
        int end = min(endMinute, (int)video_data_[videoId].size() - 1);
        return video_data_[videoId].substr(startMinute, end - startMinute + 1);
    }
    ...
    vector<int> getLikesAndDislikes(int videoId) {
        ...
        return {like_cnt_[videoId], dislike_cnt_[videoId]};
    }
