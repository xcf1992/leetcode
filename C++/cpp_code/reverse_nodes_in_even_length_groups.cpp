...
class Solution {
private:
    ListNode* reverse(ListNode* head, ListNode* last_node) {
        if (head == nullptr || head->next == last_node) {
            return head;
        }

        ListNode* prev = last_node;
        ListNode* cur = head;
        while (cur != last_node) {
...
public:
    ListNode* reverseEvenLengthGroups(ListNode* head) {
        ...
        while (cur != nullptr) {
            int cur_cnt = 0;
            ListNode* last = cur;
            ListNode* group_end = prev;
            while (last != nullptr && cur_cnt < group_cnt) {
                last = last->next;
