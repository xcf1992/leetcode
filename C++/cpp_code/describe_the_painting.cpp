...
The colors in the overlapping segments of the painting were mixed when it was painted. When two or more colors mix, they
form a new color that can be represented as a set of mixed colors.
...
These segments can be represented by the 2D array painting where painting[j] = [leftj, rightj, mixj] describes a
...
because: [1,4) is colored {5,7} (with a sum of 12) from both the first and second segments. [4,7) is colored {7} from
only the second segment. Return the 2D array painting describing the finished painting (excluding any parts that are not
painted). You may return the segments in any order.
...
public:
    vector<vector<long long>> splitPainting(vector<vector<int>>& segments) {
        ...
        long long cur_sum = 0;
        int cur_pos = -1;
        for (auto& [pos, sum] : colors) {
            if (cur_sum != 0) {
                rst.push_back({cur_pos, pos, cur_sum});
            }
            cur_pos = pos;
            cur_sum += sum;
        }
