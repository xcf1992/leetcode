...
In imprecise terms, two objects a and b are considered equivalent if neither compares less than the other: !comp(a, b)
and !comp(b, a).

The order of the elements that compare equivalent is the order of insertion and does not change. (since C++11)

std::multiset meets the requirements of Container, AllocatorAwareContainer, AssociativeContainer and
ReversibleContainer.
*/
class Solution {
public:
...
public:
    vector<double> medianSlidingWindow(vector<int>& nums, int k) {
        ...
        for (int i = 0; i < nums.size() - k + 1; i++) {
            result[i] =
                    k % 2 == 0 ? (static_cast<double>(window[k / 2 - 1]) + (double)window[k / 2]) / 2 : window[k / 2];
            if (i + k < nums.size()) {
