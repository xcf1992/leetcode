...
int largestMatrix() Returns the index x where matrix[x] contains the most number of 1's. If there are multiple such
indices, return the largest x.
...
matrix3D.largestMatrix(); // Returns 3. All indices from 0 to 3 tie with the same number of 1's, but index 3 is the
biggest.
...
class Matrix3D {
private:
    vector<vector<vector<int>>> matrix_;
    set<pair<int, int>> cnt_idx_;
    vector<int> cnt_;
public:
    Matrix3D(int n) {
        cnt_.resize(n, 0);
        matrix_.resize(n, vector<vector<int>>(n, vector<int>(n, 0)));
        for (int i = 0; i < n; i++) {
            cnt_idx_.insert({0, i});
        }
    }
    ...
    void setCell(int x, int y, int z) {
        if (matrix_[x][y][z] == 0) {
            int cur_cnt = cnt_[x];
            cnt_idx_.erase({cur_cnt, x});
            cnt_[x] += 1;
            cnt_idx_.insert({cnt_[x], x});
        }
        matrix_[x][y][z] = 1;
    }
