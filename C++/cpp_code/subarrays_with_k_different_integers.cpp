...
Time complexity O(NK), each element is put/pop from the sliding windows/map only once, each iteration might loop throguh
all k element in the map. Space complexity O(K), for the map keep track of last apperance of each K element.
*/
class Solution1 {
public:
    int subarraysWithKDistinct(vector<int>& A, int K) {
