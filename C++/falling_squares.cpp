...
/*
 ...
 3. for each droped square we need to update height from index[pos.first] to index[pos.first + pos.second - 1],
 ...
 4. then we need to update new height from index[pos.first] to index[pos.first + pos.second - 1].
...
public:
    vector<int> fallingSquares(vector<pair<int, int>>& positions) {
        ...
        for (pair<int, int>& pos : positions) {
            coords.insert(pos.first);
            coords.insert(pos.first + pos.second - 1);
        }
        ...
        int current = 0;
        for (pair<int, int>& pos : positions) {
            int left = index[pos.first];
            int right = index[pos.first + pos.second - 1];
            int height = query(left, right, heights) + pos.second;
