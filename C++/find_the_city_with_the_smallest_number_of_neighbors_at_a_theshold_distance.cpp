...
and weighted edge between cities fromi and toi,
...
we can use Floyd-Warshall algorithm first to get distances between all pairs of nodes.
Then just iterate through each node and count the number of nodes that meet the requirements.
...
public:
    int findTheCity(int n, vector<vector<int>>& edges, int distanceThreshold) {
        vector<vector<long>> distance(n, vector<long>(n, INT_MAX));
