...
/*
 ...
 The number of eggs will determine the strategy you can use to check the floors. if you have only one egg, you need to use the most conservative strategy. which is check from level 1; and move 1 level up each time, then when the egg broke, you can determine the target level. Otherwise if the egg broke, you will have no egg to perform another test.
 If you have more eggs, you can try some other aggressive strategy; which will probably decease the number of moves needed
 ...
 The basic idea is when the egg break, we will search dp[m - 1][k - 1] below current level and when the egg not break we will search dp[m - 1][k] above current level. That's the reason why we will combine them together but not only pick the smaller one. Hope my explanation can help you understand Lee's solution better.
