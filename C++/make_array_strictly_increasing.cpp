...
private:
    ...
    int dfs(int cur, int prev, vector<int>& arr1, vector<int>& arr2) {
        ...
        if (it != arr2.end()) { // we may still need to replace arr1[cur], even when *it > arr1[cur] as it may happen we replaced arr1[cur - 1], so now arr1[cur] is not big enough, case: [1,5,3,6,7], [1,3,4]
